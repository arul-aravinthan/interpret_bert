...
class PyTorchClassifier(object):
  ...
  def prepare_split(self, X, y, validation_data=None, validation_split=None):
    ...
    trainX = torch.as_tensor(trainX).to(device, dtype=torch.float32)
    trainy = torch.as_tensor(trainy).to(device, dtype=torch.int64)
    devX = torch.as_tensor(devX).to(device, dtype=torch.float32)
    devy = torch.as_tensor(devy).to(device, dtype=torch.int64)
  ...
  def trainepoch(self, X, y, epoch_size=1):
    ...
    for _ in range(self.nepoch, self.nepoch + epoch_size):
      ...
      for i in range(0, len(X), self.batch_size):
        # forward
        idx = torch.as_tensor(permutation[i:i + self.batch_size]).long().to(X.device)
        Xbatch = X[idx]
        ybatch = y[idx]
  ...
  def score(self, devX, devy):
    ...
    devX = torch.as_tensor(devX).cuda()
    devy = torch.as_tensor(devy).cuda()
    with torch.no_grad():
